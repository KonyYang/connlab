...
4. Read `docs/project_management/PARALLEL_EXECUTION_MODEL.md` when `docs/task_board.md` uses controlled parallel lanes.
5. Create a concrete implementation plan document first when the task is non-trivial (scope, design, file-level changes, risks, validation).
6. Submit the plan document for user review and wait for explicit approval.
7. After user approval, implement only the current active task or approved active lane allowed by `docs/task_board.md`.
8. Add/update tests for functional code tasks.
9. Run relevant tests or document why validation is documentation-only.
10. Record lane evidence when working in controlled-parallel mode.
11. Update `docs/task_board.md` only when your role is allowed to update it.
12. Summarize changed files and any known limitations.
...
1. Read order for every new task:
   ...
   - `docs/project_management/PARALLEL_EXECUTION_MODEL.md` when controlled-parallel mode is active or proposed
   - any task-specific docs referenced by that task
2. Do not start a task or lane that is not marked as current, explicitly ready, or approved in `docs/task_board.md`.
3. Do not skip forward to later tasks just because they look implementable.
4. In single-task mode, when a task is completed, update `docs/task_board.md` in the same turn:
   - task status
   - last updated date
   - completion notes
   - next recommended task
   - validation summary
5. In controlled-parallel mode:
   - Planner may update lane definitions, planned status, and approval state after user approval.
   - Developer may update only lane-local evidence files when the lane permits it; Developer must not change global current phase, global next task, unrelated lane rows, or merge status.
   - Reviewer may write review evidence and findings; Reviewer must not merge or silently fix code unless explicitly reassigned as Developer.
   - QA / Smoke Owner may write smoke evidence; QA must not modify product code.
   - Integrator owns authoritative lane completion, merge status, final validation summary, and global `docs/task_board.md` updates.
6. If implementation reality differs from the board, update the board only through the role allowed for that execution mode instead of relying on conversational memory.

Execution priority:

- Stable rules and scope boundaries: `AGENTS.md`
- Current stage, active task, lane definitions, and completion state: `docs/task_board.md`
- Concrete implementation details: current `tasks/TASK_XXX_*.md`
- Controlled-parallel operating rules: `docs/project_management/PARALLEL_EXECUTION_MODEL.md`
...
- current phase
- current active task ID or approved lane ID
- current role when controlled-parallel mode is active
- why this task or lane is allowed now

If the requested task is ahead of the active task or outside the approved lane on the board, stop and report the mismatch instead of implementing it silently.

## 16. Controlled Parallel Execution
...
3. Every lane must declare task id, owner role, branch/worktree, dependencies, conflict scope, `May Touch`, `Must Not Touch`, `Locked Paths`, evidence file, validation gate, and merge gate.
4. Executors must not edit files outside their lane scope.
5. Parallel lanes touching the same authority path, Office gateway, database schema, lifecycle state, task board global state, or locked path must be serialized unless Planner explicitly marks the boundary safe.
6. Every lane must maintain role-appropriate evidence under `docs/lane_evidence/`.
7. After a lane completes, it enters review; it must not be merged until Reviewer and Integrator gates pass.
8. Proposed/planned lanes are not executable. Only approved lanes may be implemented.

### Role Mismatch Protocol
...
3. controlled-parallel 模式下还必须阅读 `docs/project_management/PARALLEL_EXECUTION_MODEL.md`
4. 先产出“可执行方案文件”（必须可审阅，包含范围、改动点、风险、验证），除非当前任务明确是已批准的文档模板落地任务
5. 将方案文件提交给用户审阅，未获得“同意/批准”前禁止写实现代码
6. 用户批准后再进入第二步：实施编码与测试
7. 执行 `docs/project_management/TASK_REVIEW_CHECKLIST.md`
8. 提供运行验证方法
9. controlled-parallel 模式下写入对应 lane evidence
10. 停止，不进入下一个任务或 lane
...
* 不允许实现未指定功能
* 不允许跨 Task 或跨未批准 lane 开发
* 不允许自动推进多个 Task 或 lane
* 不允许重构未授权代码
* 不允许角色静默执行其他角色职责
...
* 执行者（不是架构师），除非当前对话明确设定为 Planner / Reviewer / Integrator / QA
* 必须服从 Task
* 必须服从 AGENTS.md
* controlled-parallel 模式下必须服从 lane scope、role scope、evidence 和 merge gate
