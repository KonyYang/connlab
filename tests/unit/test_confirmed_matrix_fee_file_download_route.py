...
def test_fee_file_download_path_guard_accepts_xlsx_inside_download_dir(
    tmp_path: Path,
) -> None:
...
def test_fee_file_download_path_guard_rejects_non_xlsx_inside_download_dir(
    tmp_path: Path,
) -> None:
    download_dir = tmp_path / "generated_fee_files"
    download_dir.mkdir()
    ...
    assert exc_info.value.status_code == 500
    assert "expected a .xlsx workbook" in str(exc_info.value.detail)
