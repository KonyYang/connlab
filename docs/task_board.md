...
- `TASK_257_CONFIRMED_MATRIX_AUTHORITY_MODEL` is planned. Task file and executable plan define the next controlled backend authority boundary: confirming one saved Project Matrix Draft into immutable active Confirmed Matrix authority, while deferring revision flow, frontend confirm wiring, runtime execution, StepInstance, reports, fee, duration, equipment, and Confirmed Step Output generation. Planning boundaries now explicitly fix first-slice `confirmed_revision = 1`, require DB-level active-authority uniqueness, copy all non-sample draft rows, keep draft content/status read-only during confirmation, and reject blank selected group key/label.
- Deliverables planned for review: `tasks/TASK_257_CONFIRMED_MATRIX_AUTHORITY_MODEL.md` and `docs/task_257_confirmed_matrix_authority_model_plan.md`.
- Next action: review and explicitly approve TASK_257 before implementation. Do not implement until approval is recorded.
