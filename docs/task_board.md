...
- `TASK_262_MATRIX_IMPORT_GROUP_SELECTION_VIEW` is planned. The task file and executable plan define the frontend Group Selection View that appears after Matrix import preview confirmation, calls the TASK_261 commit API, and loads the returned selected-only ProjectMatrixDraft into Matrix Editor.
- Deliverables planned: `tasks/TASK_262_MATRIX_IMPORT_GROUP_SELECTION_VIEW.md` and `docs/task_262_matrix_import_group_selection_view_plan.md`.
- Next action: review and explicitly approve `TASK_262_MATRIX_IMPORT_GROUP_SELECTION_VIEW` before any implementation code is written.
