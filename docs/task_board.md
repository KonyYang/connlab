...
- `TASK_288_FEE_EVALUATION_EXCEL_EXPORT` is planned as the next follow-up and awaits explicit approval. Scope: export reviewed fee draft data into the official fee workbook template, default `Prepared by` from ConnLab login user first then Windows/computer user, leave `Approved by` for manual entry, allow `.xlsx` fallback when `.xls` automation is unavailable, and update `ProjectOutputRecord` lineage/freshness. Boundary: no rule-maintenance UI, no StepInstance/execution persistence, and no report expansion.
