...
- `TASK_293_FEE_EVALUATION_EXCEL_PREVIEW_UI` is planned and awaiting explicit approval. The task and executable plan refocus `/projects/:projectId/fee-evaluation` into an Excel-preview-first surface modeled on the real `Testing Prices` form, with the current rule review table moved to secondary `Review details`. Planned scope is frontend-only and keeps TASK_290/TASK_291 Matrix basic-fill export behavior unchanged. Plan files: `tasks/TASK_293_FEE_EVALUATION_EXCEL_PREVIEW_UI.md` and `docs/task_293_fee_evaluation_excel_preview_ui_plan.md`.
