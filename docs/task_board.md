...
- `TASK_256_MATRIX_EDITOR_SAVE_TO_DRAFT_WIRING` is planned and awaiting user approval. The task will connect Matrix Editor's current editable state to structured Project Matrix Draft persistence with draft-only save semantics, typed frontend API client wiring, save state feedback, sparse non-empty cell handling, group-level sample quantity persistence, and source immutability checks. It explicitly excludes confirmed authority, Matrix revision flow, runtime execution, StepInstance persistence, reports, fee, duration, equipment assessment, and major Matrix Editor redesign.
