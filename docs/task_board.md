...
- `TASK_255_PROJECT_MATRIX_DRAFT_PERSISTENCE_MODEL` is planned and awaiting user approval. The task will implement editable Project Matrix Draft persistence from immutable Source Matrix snapshots, including selected draft groups and group-level sample quantity as draft-local working-copy state. It explicitly excludes confirmed authority, Matrix revision flow, Matrix Editor frontend save wiring, runtime execution, StepInstance persistence, reports, fee, duration, and equipment assessment.
