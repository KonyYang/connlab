...
> Status: TASK_149 complete + ... + TASK_321 complete + TASK_314 split accepted + TASK_314A complete
> Last Updated: 2026-06-14
> Current Source Of Truth: `docs/task_board.md`
> Current Active Task: `TASK_314A_MATRIX_EDITOR_DRAFT_PERSISTENCE` is complete. Next work requires separate user approval: either proceed to `TASK_315_MATRIX_DRAFT_TO_FEE_DRAFT_INCREMENTAL_REBASE` now that its real Matrix prerequisite is complete, or approve future `TASK_314B_FEE_EVALUATION_BACKGROUND_DRAFT_PERSISTENCE`. `TASK_314C_MATRIX_FEE_PROJECT_FOLDER_REGRESSION` remains deferred. Do not implement code or enter a later task without separate user approval.
...
- `TASK_314_MATRIX_AND_FEE_BACKGROUND_DRAFT_PERSISTENCE` is now an umbrella/historical split rationale, not an executable implementation task. The accepted split is `TASK_314A` Matrix Editor draft persistence first, future `TASK_314B` Fee Evaluation background draft persistence, and future `TASK_314C` Matrix/Fee/Project Folder regression. If existing unverified worktree changes attempted the combined TASK_314 scope, review them against TASK_314A before continuing and defer Fee Evaluation autosave changes unless TASK_314B is explicitly approved.
