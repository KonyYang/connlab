...
- `TASK_277_MATRIX_EDITOR_SINGLE_DRAFT_PUBLISH_FLOW` is planned. This Matrix Editor follow-up will collapse the user-facing workflow into one auto-saved draft editing surface and one `Confirm As Active Matrix` publish action. It will remove exposed internal revision concepts such as `Create Revision Draft`, `Confirm Revision`, `Draft Actions`, `Authority Actions`, large `Current State` banners, and action consequence paragraphs; compact the Matrix Editor header; keep `Undo`, document import, and selected-group adjustment as Matrix toolbar actions; internally use existing first-confirm/revision-confirm APIs as appropriate; flush unsaved edits before confirm; avoid no-change publishes; and return to Workbench after successful confirm so the projection reloads. Planned deliverables: `frontend/src/features/matrix-editor/MatrixEditorWorkspace.tsx`, `frontend/src/features/matrix-editor/MatrixWorkspaceActionGroups.tsx`, `frontend/src/features/matrix-editor/MatrixWorkspaceStateBanner.tsx`, `frontend/src/features/matrix-editor/matrixWorkspaceClarityModel.ts`, `frontend/src/features/matrix-editor/MatrixEditorWorkspace.test.tsx`, `frontend/src/workbench.css`, `tests/unit/test_frontend_shell_files.py`, `tasks/TASK_277_MATRIX_EDITOR_SINGLE_DRAFT_PUBLISH_FLOW.md`, and `docs/task_277_matrix_editor_single_draft_publish_flow_plan.md`. Scope boundary: frontend-first; no cross-project import, historical version picker, StepInstance, Test Record generation, report/fee/image/evidence/AI/permission work, or backend schema change unless an explicit backend contract gap is approved.
