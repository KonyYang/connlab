...
- Next recommended action: review and approve `TASK_256_MATRIX_EDITOR_SAVE_TO_DRAFT_WIRING` before implementing Matrix Editor save-to-draft wiring.

- `TASK_256_MATRIX_EDITOR_SAVE_TO_DRAFT_WIRING` is planned and awaiting user approval. The task will connect Matrix Editor's current editable state to structured Project Matrix Draft persistence with draft-only save semantics, typed frontend API client wiring, save state feedback, sparse non-empty cell handling, group-level sample quantity persistence, and source immutability checks. Planning constraints now define last-write-wins concurrency for this task, a persisted-field whitelist (`rows`, `groups`, `cells`, `selected groups`, group-level sample quantity), no UI-only state persistence, disabled Save behavior when no `project_matrix_draft_id` exists, no implicit draft creation from Save, stable error mapping (`404`, `409`, `422`, `500`), rollback testing, and task-board closeout requirements. It explicitly excludes confirmed authority, Matrix revision flow, optimistic concurrency/version checks, runtime execution, StepInstance persistence, reports, fee, duration, equipment assessment, and major Matrix Editor redesign.
- Deliverables planned: `tasks/TASK_256_MATRIX_EDITOR_SAVE_TO_DRAFT_WIRING.md`, `docs/task_256_matrix_editor_save_to_draft_wiring_plan.md`, backend draft update API/service/repository updates as needed, frontend API client wiring, Matrix Editor save-state wiring, and focused backend/frontend tests.
