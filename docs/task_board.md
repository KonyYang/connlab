...
- `TASK_278_MATRIX_EDITOR_TEMPORARY_SESSION_FLOW` is planned and awaiting user approval. Goal: convert Matrix Editor into a temporary editing session based on the current Workbench Active Matrix plus its full SourceMatrix snapshot. Visible actions become `Cancel` and `Confirm Matrix`; cancel discards the current session; confirm publishes changed content back to Workbench; next entry never resumes unconfirmed edits; `Change selected Groups` uses the full source snapshot so all source groups remain selectable. Plan file: `docs/task_278_matrix_editor_temporary_session_flow_plan.md`. Task file: `tasks/TASK_278_MATRIX_EDITOR_TEMPORARY_SESSION_FLOW.md`. Scope boundary: no saved draft restore UI, no StepInstance/execution persistence, no image/evidence/report/fee/AI/permission scope, and no new table unless plan review proves it unavoidable and user approves.
