...
- `TASK_261_MATRIX_IMPORT_GROUP_SELECTION_COMMIT` is planned. The task file and executable plan define the backend-only import commit boundary that persists full Source Matrix lineage while creating a selected-only ProjectMatrixDraft from selected group keys.
- Deliverables planned: `tasks/TASK_261_MATRIX_IMPORT_GROUP_SELECTION_COMMIT.md` and `docs/task_261_matrix_import_group_selection_commit_plan.md`.
- Next action: review and explicitly approve `TASK_261_MATRIX_IMPORT_GROUP_SELECTION_COMMIT` before any implementation code is written.
