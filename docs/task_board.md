...
> Status: TASK_149 complete + ... + TASK_321 complete + TASK_314 split accepted + TASK_314A complete + TASK_314B complete + TASK_314C complete + TASK_315 split accepted + TASK_315A complete + TASK_315B complete
> Last Updated: 2026-06-15
> Current Source Of Truth: `docs/task_board.md`
> Current Active Task: `TASK_315B_PENDING_REBASE_PERSISTENCE_AND_MATRIX_AUTOSAVE_CANCEL_LIFECYCLE` is complete after separate explicit user approval. `TASK_315C_MATRIX_CONFIRM_PROMOTION` now has a reviewable task file and plan, but remains unapproved for implementation. `TASK_315_MATRIX_DRAFT_TO_FEE_DRAFT_INCREMENTAL_REBASE` remains an umbrella/split rationale, not a direct implementation task. Do not implement TASK_315C, TASK_315D, Matrix Confirm promotion, Fee UI, Project Folder, StepInstance/report/AI/permissions/LAN/multi-user scope without separate approval.
...
- `TASK_315C_MATRIX_CONFIRM_PROMOTION` is planned with a reviewable task file and plan. Scope is Matrix Confirm promotion of pending rebase into a current Fee pricing draft for the newly confirmed Matrix context, synchronous fallback when pending is missing, and non-blocking failure reporting when Fee promotion fails after Matrix Confirm succeeds. It must not implement Fee Evaluation UI, inactive removed-row display, Project Folder behavior, Confirm Fee changes, frontend status UI, StepInstance/report/AI/permissions/LAN/multi-user work. Task file: `tasks/TASK_315C_MATRIX_CONFIRM_PROMOTION.md`. Plan: `docs/task_315c_matrix_confirm_promotion_plan.md`. Implementation requires separate explicit approval.
