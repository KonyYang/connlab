...
- `TASK_285_FEE_RULE_SEED_LIBRARY` is planned as a follow-up after TASK_285A. Scope: create a versioned fee rule seed library from `Unit Price Reference`, with source/version metadata, conservative deterministic matcher support, duplicate-alias validation, and tests. Boundary: no Matrix-to-fee generation, no UI, no Excel export, no SQLite rule persistence, and no AI extraction.
- `TASK_286_CONFIRMED_MATRIX_TO_FEE_DRAFT` is planned as a follow-up blocked by TASK_285. Scope: generate a backend `FeeEvaluationDraft` preview from active Confirmed Matrix authority and the active fee rule version, with rule-version traceability and review-required warnings. Boundary: no frontend review UI, no Excel export, and no persisted edited fee draft.
...
- `TASK_288_FEE_EVALUATION_EXCEL_EXPORT` is planned as a follow-up blocked by TASK_287. Scope: export reviewed fee draft data into the official fee workbook template and update `ProjectOutputRecord` lineage/freshness. Boundary: no rule-maintenance UI, no StepInstance/execution persistence, and no report expansion.
