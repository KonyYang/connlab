...
- `TASK_286_CONFIRMED_MATRIX_TO_FEE_DRAFT` is planned as the next follow-up and awaits explicit approval. Scope: generate a backend `FeeEvaluationDraft` preview from active Confirmed Matrix authority and the active fee rule version, with rule-version traceability, concrete `pricing_effective_from` from project sample received date, and review-required warnings. Boundary: no frontend review UI, no Excel export, and no persisted edited fee draft.
- `TASK_287_FEE_EVALUATION_REVIEW_UI` is planned as a follow-up blocked by TASK_286. Scope: add a Workbench derived-output review surface for the fee draft, showing line items, matched rules, totals, and review-required reasons. Boundary: no Excel export, no rule-maintenance UI, and no Matrix authority editing.
- `TASK_288_FEE_EVALUATION_EXCEL_EXPORT` is planned as a follow-up blocked by TASK_287. Scope: export reviewed fee draft data into the official fee workbook template, default `Prepared by` from ConnLab login user first then Windows/computer user, leave `Approved by` for manual entry, allow `.xlsx` fallback when `.xls` automation is unavailable, and update `ProjectOutputRecord` lineage/freshness. Boundary: no rule-maintenance UI, no StepInstance/execution persistence, and no report expansion.
