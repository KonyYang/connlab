...
- Next recommended action: review and approve `TASK_255_PROJECT_MATRIX_DRAFT_PERSISTENCE_MODEL` before implementing editable Project Matrix Draft persistence.

- `TASK_255_PROJECT_MATRIX_DRAFT_PERSISTENCE_MODEL` is planned and awaiting user approval. The task will implement editable Project Matrix Draft persistence from immutable Source Matrix snapshots, including selected draft groups and group-level sample quantity as draft-local working-copy state. Added planning constraints cover atomic draft creation rollback, duplicate `project_id + source_import_id` conflict handling, selected-group defaults, group-level sample quantity as single draft truth, sparse non-empty draft cells with empty-as-delete semantics for future updates, group/row lineage IDs, API-to-application-only routing, and rollback testing. It explicitly excludes confirmed authority, Matrix revision flow, Matrix Editor frontend save wiring, runtime execution, StepInstance persistence, reports, fee, duration, and equipment assessment.
- Deliverables planned: `tasks/TASK_255_PROJECT_MATRIX_DRAFT_PERSISTENCE_MODEL.md`, `docs/task_255_project_matrix_draft_persistence_model_plan.md`, backend domain/storage/repository/application-service files, and focused unit/integration tests.
