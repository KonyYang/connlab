...
- `TASK_271_TEST_RECORD_WORD_GENERATION_V1` is planned. Scope is controlled Matrix-to-Test-Record continuity: generate a downloadable Word `.docx` Test Record draft from active ConfirmedMatrix only, using selected groups and confirmed step data. Plan: `docs/task_271_test_record_word_generation_v1_plan.md`. Task file: `tasks/TASK_271_TEST_RECORD_WORD_GENERATION_V1.md`. Awaiting explicit user approval before implementation. Scope boundary: no formal TestRecord aggregate, StepInstance/execution persistence, report engine, fee engine, AI, equipment, permission, generation history, historical template selection, generic template engine, project-folder managed save, or Matrix authority mutation.
