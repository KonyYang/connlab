...
- Existing `DefaultMatrixFeePendingRebaseBuilder` for fallback.
...
### Draft-To-Confirmed Id Remapping

TASK_315B pending active rows are target-draft-shaped. They may contain `confirmed_group_id`/`confirmed_row_id` values that are actually Matrix draft ids. After Matrix Confirm, the new Confirmed Matrix uses fresh confirmed ids:

- `ConfirmedMatrixGroup.draft_group_id` maps draft group id to new `confirmed_group_id`.
- `ConfirmedMatrixRow.draft_row_id` maps draft row id to new `confirmed_row_id`.

Promotion must remap every active row:

```python
row.confirmed_group_id = group_by_draft_id[row.confirmed_group_id].confirmed_group_id
row.confirmed_row_id = row_by_draft_id[row.confirmed_row_id].confirmed_row_id
```
...
4. If pending is missing/stale/unusable:
   ...
   - otherwise run fallback rebase via `DefaultMatrixFeePendingRebaseBuilder` using the saved Matrix draft and previous active Matrix context;
...
- [ ] **Step 1: Write failing deserialization/remapping tests**

Add tests for:

- `pending_rebase_payload_from_json(...)` roundtrips rows/manual rows/summary/warnings produced by `pending_rebase_payload_to_json(...)`.
- remapping replaces draft group/row ids with new Confirmed Matrix group/row ids.
...
- [ ] **Step 2: Implement deserializer and remapping helpers**
...
- summary should use existing `FeeEvaluationEditedExportSummary` defaults from source pricing draft when available, or safe blank defaults if pending payload has no Fee summary fields;
...
- [ ] **Step 1: Write failing promotion service tests**

Cover:

- valid pending rebase is promoted into new pricing draft context;
- promoted rows use new Confirmed Matrix ids;
...
- [ ] **Step 1: Write failing Matrix Confirm integration tests**
...
- [ ] **Step 2: Extend MatrixEditorSessionConfirmResult**
...
promotion_service = MatrixFeeRebasePromotionService(
    ...
    fallback_builder=DefaultMatrixFeePendingRebaseBuilder(...),
)
```

Pass it into `MatrixEditorSessionService`.
...
- fallback promotion uses current base pricing draft and saves to new Matrix context;
...
- [ ] Does Matrix Confirm still succeed when Fee promotion fails?
- [ ] Does promotion write a current Fee pricing draft for the new Confirmed Matrix id/revision?
- [ ] Are promoted rows remapped to new Confirmed Matrix ids?
- [ ] Are inactive removed rows kept out of active pricing draft rows?
- [ ] Is pending deleted only after successful promotion?
- [ ] Does fallback avoid blocking Matrix Confirm?
- [ ] Are Confirm Fee, Fee UI, Project Folder, and frontend untouched?
