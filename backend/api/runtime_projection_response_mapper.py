"""Shared runtime projection response DTOs and mapping helpers."""
...
from backend.modules.runtime_projection.snapshot_adapter import RuntimeProjectionSnapshot
...
def to_runtime_projection_read_only_snapshot_response(
    snapshot: RuntimeProjectionSnapshot,
) -> RuntimeProjectionReadOnlySnapshotResponse:
    """Convert one runtime projection snapshot dataclass into API response DTOs."""
...
def _value_count_items(
    values: list[list[str | None | int]] | tuple[tuple[str | None, int], ...],
) -> list[ValueCountItemResponse]:
...
def _step_workspace_response(
    workspace_data: dict | None,
) -> StepWorkspaceConsumerResponse | None:
