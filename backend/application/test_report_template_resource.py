...
def resolve_customer_report_template_path(
    resource_store: TestReportTemplateResourceStore,
) -> Path:
    """Return the unique E-4515 `.docx` under Settings > Template folder."""
    resource = resource_store.get_by_type(ExternalResourceType.PROJECT_FOLDER_TEMPLATE)
    if resource is None:
        raise TestReportTemplateResourceError("Template folder is not configured.")
    if not resource.active:
        raise TestReportTemplateResourceError("Template folder is inactive.")
    return discover_customer_report_template(Path(resource.path))
...
def discover_customer_report_template(template_folder: Path) -> Path:
    """Discover exactly one approved E-4515 customer-report template."""
    folder = Path(template_folder)
    if not folder.is_dir():
        raise TestReportTemplateResourceError(
            f"Template folder does not exist: {folder}"
    ...
    matches = sorted(
        ...
        and not path.name.startswith("~$")
        and path.suffix.lower() == ".docx"
        and "E-4515" in path.name.upper()
    )
    if not matches:
        raise TestReportTemplateResourceError(
            "E-4515 customer report template not found in Template folder."
        )
    if len(matches) > 1:
        names = ", ".join(path.name for path in matches)
        raise TestReportTemplateResourceError(
            f"Multiple E-4515 customer report templates found in Template folder: {names}"
        )
    return matches[0]
